...
def laser_start_end(laser, SR=1000., intval=5, offset=0):
    """
    ...
    if len(idx) == 0 :
        #return (None, None)
        return ([], [])
    idx = np.array(([int(i+(offset*SR)) for i in idx]))
...
def my_hpfilter(x, w0, N=4):
    ...
    # w0 is scaled from 0 to 1, with 1 being the Nyquist frequency (sr/2)
    # e.g. for sr=100 and w0=0.5, the Nyquist freq is 500, and the cutoff freq is w0 * (sr/2) = 250

    b,a = signal.butter(N, w0, 'high')  # get numerator & denominator polynomials of IIR filter
    y = signal.filtfilt(b, a, x)
...
#     # smoothed = scipy.signal.fftconvolve(x, F, 'same')
#     # # replace edges of convolved data with original samples to prevent boundary discontinuity
#     # if edge > 0:
#     #     smoothed[0:int(edge/2)] = x[0:int(edge/2)]
#     #     smoothed[-int(edge/2) : ] = x[-int(edge/2) : ]
    
#     # convolve input with Gaussian, input edges replicated by 1 element to avoid boundary discontinuity
#     smoothed = scipy.ndimage.convolve1d(x, F, mode='nearest')
    ...
    for i in range(ntbins):
        ...
        if prem == 0 and pow_delta[i] < thr_delta and pow_mu[i] < thr_mu:
            ...
            if th_delta[i] > thr_th_delta1:
                ...
                c_mu = np.sum( np.where(pow_mu[int(sstart):i]>thr_mu)[0] ) / past_len
...
def rem_online_analysis(ppath, recordings, plotMode='0', single_mode=False, fig_file='', backup='', overlap=0, ylim=[]):
    """
    ...
    :param ylim: set lower and upper bounds of y-axis as tuple - ex. (0, 100)
    :return: df (pd.DataFrame, with control and experimental REM durations as data columns) and independent samples ttest
    """
    ...
    df = pd.DataFrame({'mice':data['mice'], 'ctr':pd.Series(data['ctr']), 'exp':pd.Series(data['exp'])})
    ...
    if len(plotMode) > 0:
        ...
        #data_yerr = [df['ctr'].std(axis=0), df['exp'].std(axis=0)]  # std
        data_yerr = [df['ctr'].std(axis=0) / np.sqrt(len(df['ctr'])), df['exp'].std(axis=0) / np.sqrt(len(df['exp']))]  # sem
    
        if '0' in plotMode:
            ax.bar([0,1], data_plot, yerr=data_yerr, align='center', color=['gray', 'blue'], edgecolor='black')
        else:
        ...
        if '5' in plotMode:
            ...
            for mrow, mname in enumerate(mice):
                ...
                if '1' in plotMode or '2' in plotMode:
                    ax.plot([0,1], points, color=markercolor, marker='o', ms=8, markeredgewidth=2, linewidth=0, markeredgecolor='black', label=mname, clip_on=False)
...
def cl_ctr_vs_exp(ppath, ctr_rec, exp_rec, istate, stat, plotMode='0', overlap=0, ma_thr=20,
                            ma_state=3, flatten_tnrem=False, mouse_avg='mouse', ylim=[], ci=68):
    """
    Compare brainstate stats between control and experimental mouse groups in closed-loop experiment
    :param ctr_rec: list of control recordings
    :param exp_rec: list of experimental recordings
    :param istate: brainstate to compare
    :param stat: if 'dur' - compare average duration of brainstate between all bouts in control vs experimental animals
                  if 'perc' - compare percent time spent in brainstate across entire recording in control vs experimental animals
    """
    from scipy import stats
    if type(ctr_rec) != list: ctr_rec = [ctr_rec]
    if type(exp_rec) != list: exp_rec = [exp_rec]
    ...
    for rec in ctr_rec + exp_rec:
        ...
        M = AS.adjust_brainstate(M, dt, ma_thr=ma_thr, ma_state=ma_state, flatten_tnrem=flatten_tnrem)
        ...
        if stat == 'perc':
            ...
            # state sequences from offline analysis (assumed to be "ground truth")
            seq = get_sequences(np.where(M==istate)[0])
            for s in seq:
                isect = np.intersect1d(s, rem_idx)
                # test if real REM period s overlaps with online detected REM periods 
                # if yes, make sure that the overlap is at least overlap *100 percent
                if len(np.intersect1d(s, rem_idx)) > 0 and float(len(isect)) / len(s) >= overlap:
                    data.append((s[-1]-s[0]+1)*dt)
                    # if sequence overlaps with laser, append to data
                    #if len(np.intersect1d(isect, laser_idx))>0:
                        #data.append(dur)
        if rec in ctr_rec:
            cdict[rec] = data
    ...
    plt.figure()
    sns.barplot(x='group', y=stat, data=df, ci=ci, palette={'control':'gray', 'exp':'blue'})
    if mouse_avg in ['trial', 'trials']:
        sns.swarmplot(x='group', y=stat, data=df, palette={'control':'gray', 'exp':'blue'})
    elif mouse_avg in ['mouse', 'rec']:
        #sns.stripplot(x='group', y=stat, data=df, color='black', size=8)
        sns.swarmplot(x='group', y=stat, data=df, color='black', size=8)
    
    # if len(plotMode) > 0:
    #     if mouse_avg not in ['trial', 'trials']:
    #         mcs = {}
    #         for cm in cmice: mcs.update(AS.colorcode_mice(cm))
    #         for em in emice: mcs.update(AS.colorcode_mice(em))
        
    #     plt.ion()
    #     plt.figure()
    #     ax = plt.gca()
    #     data_plot = [np.nanmean(cdata), np.nanmean(edata)]
    #     data_yerr = [np.nanstd(cdata), np.nanstd(edata)]  # std
    #     #data_yerr = [np.nanstd(cdata) / np.sqrt(len(cdata)), np.nanstd(edata) / np.sqrt(len(edata))]  # sem
    ...
    #         ax.bar([0,1], data_plot, align='center', color=['gray', 'blue'], edgecolor='black')
    #     pdb.set_trace()
    #     if (mouse_avg in  ['trial', 'trials']) and ('1' in plotMode or '2' in plotMode):
    #         ax.plot(np.zeros((len(cdata),)), cdata, '.', color='black')
    #         ax.plot(np.ones((len(edata),)), edata, '.', color='black')
    #     else:
    #         for cidx, cname in enumerate(clabels):
    #             if '1' in plotMode: markercolor = 'black'
    #             elif '2' in plotMode: markercolor = mcs[cname]
    
    #             if '1' in plotMode or '2' in plotMode:
    #                 ax.plot([0], [cdata[cidx]], color=markercolor, marker='o', ms=8, markeredgewidth=2, linewidth=0, markeredgecolor='black', label=cname, clip_on=False)
            
    #         for eidx, ename in enumerate(elabels):
    #             if '1' in plotMode: markercolor = 'black'
    #             elif '2' in plotMode: markercolor = mcs[ename]
    
    #             if '1' in plotMode or '2' in plotMode:
    #                 ax.plot([1], [edata[eidx]], color=markercolor, marker='o', ms=8, markeredgewidth=2, linewidth=0, markeredgecolor='black', label=ename, clip_on=False)
            
    #     ax.set_xticks([0,1])
    #     ax.set_xticklabels(['ctr', 'exp'])
    #     if stat == 'dur':
    ...
    # unpaired t-test between control and experimental mice
    ...
    # average ctr and lsr REM durations within each mouse before averaging together
    # if not single_mode:
    #     plt.ion()
    #     plt.figure()
    ...
    #     else:
    #         plt.bar([1], [df_mean['ctr']], yerr=np.nanstd(df['ctr'])/np.sqrt(len(data['ctr'])), capsize=5, color='grey')
    #         plt.bar([2], [df_mean['exp']], yerr=np.nanstd(df['exp'])/np.sqrt(len(data['exp'])), capsize=5, color='blue')
    
    # average ctr and lsr REM durations across all recordings and mice
    # else:
    #     plt.figure()
    #     ax = plt.axes([0.2, 0.15, 0.3, 0.7])
    #     df_mean = df.mean()
    ...
    # plt.xticks([1,2], labels=['Lsr OFF', 'Lsr ON'])
    # if len(ylim) == 2:
    #     plt.ylim(ylim)
    # box_off(ax)
    # #ax.set_xticklabels(['ctr', 'exp'], rotation=30)
...
def laser_triggered_eeg(ppath, name, pre, post, f_max, pnorm=2, pplot=False, psave=False, tstart=0, tend=-1,
                        peeg2=False, vm=2.5, prune_trials=True, mu=[10, 200], trig_state=0, offset=0, harmcs=0, iplt_level=1):
    """
    ...
    idxs, idxe = laser_start_end(lsr, SR, offset=offset)
...
def laser_triggered_eeg_avg(ppath, recordings, pre, post, f_max, laser_dur, pnorm=1, pplot=True, tstart=0, tend=-1,
                            vm=[], cb_ticks=[0], mu=[10, 100], trig_state=0, fig_file='', plot_EEGonly=False, naked=False, 
                            sf=0, prune_trials=False, offset=0, harmcs=0, iplt_level=1, bands = [(0.5,4), (6,10), (11,15), (55,99)], ci=68):
    """
    ...
    :param sf: smooth factor for spectral power bands plot
    :return: n/a
    """
    # set spectral band params
    band_labels = ['delta', 'theta', 'sigma', 'gamma']
    band_colors = ['firebrick', 'limegreen', 'cyan', 'purple']
    if ci == 'sem': ci = 68
    ...
    for rec in recordings:
        idf = re.split('_', rec)[0]
        EEG, EMG, f, t = laser_triggered_eeg(ppath, rec, pre, post, f_max, mu=mu, pnorm=pnorm, pplot=False, psave=False, 
                                             tstart=tstart, tend=tend, trig_state=trig_state, prune_trials=prune_trials, 
                                             offset=offset, harmcs=harmcs, iplt_level=iplt_level)
        EEGSpec[idf].append(EEG)
        EMGSpec[idf].append(EMG)
    
    PwrBands = {b:np.zeros((len(mice), len(t))) for b in bands}  # each band has a matrix of mice x time bins for avg power
    
    for row, idf in enumerate(mice):
        ms_sp = np.array(EEGSpec[idf]).mean(axis=0)
        ms_emg = np.array(EMGSpec[idf]).mean(axis=0)
        # calculate mean power of each band from averaged SP
        for b in bands:
            ...
            if sf > 0:
                ...
                #ms_band = AS.convolve_data(ms_band, sf)
    ...
    mf = np.where((f >= mu[0]) & (f <= mu[1]))[0]  # indices of frequencies between [mu_lo, mu_hi]
    if harmcs > 0:
        harm_freq = np.arange(0, f.max(), harmcs)  # list of harmonic frequencies
        for h in harm_freq:
            mf = np.setdiff1d(mf, mf[np.where(f[mf]==h)[0]])  # indices of non-harmonic frequencies between [mu_lo, mu_hi]
    ...
    # compile dataframe with all baseline and laser values of spectral bands
    ilsr   = np.where((t>=0) & (t<=laser_dur))[0]
    ibase  = np.where((t>=-laser_dur) & (t<0))[0]
    iafter = np.where((t>=laser_dur) & (t<laser_dur*2))[0]

    m = mice + mice + mice
    lsr  = np.concatenate((np.ones((len(mice),), dtype='int'), np.zeros((len(mice),), dtype='int'), np.ones((len(mice),), dtype='int')*2))
    ...
    for b,l in zip(bands, band_labels):
        ...
        b_pwr = np.concatenate((lsr_data, base_data, post_data))  # spectral band powers
        b_label = [l]*len(b_pwr)  # spectral band name
        # each band has a column with power values (for paired t-tests)
        df[l] = b_pwr
        # the 'Pwr' column holds all the power values, and the 'Band' column holds the band label
        df2 = df2.append(pd.DataFrame({'Mouse' : m, 'Band' : b_label, 'Pwr' : b_pwr, 'Lsr' : lsr_char}))

    # # create point plot with average power for each spectral band during pre-laser vs. laser interval
    plot_bars=True
    if plot_bars:
        plt.figure()
        ...
        for i in range(len(band_labels)):
            ...
            #sns.barplot(x='Lsr', y='Pwr', order=['PRE', 'LSR'], data=bdf, ci=ci, palette={'PRE':'gray', 'LSR':'blue'}, ax=axs[i])
            #sns.boxplot(x='Lsr', y='Perc', order=['PRE', 'LSR'], data=sdf, fliersize=0, palette={'PRE':'gray', 'LSR':'blue'}, ax=axs[i])
            sns.pointplot(x='Lsr', y='Pwr', order=['PRE', 'LSR'], data=bdf, markers='o', ci=ci, palette={'PRE':'gray', 'LSR':'blue'}, ax=axs[i])
            sns.pointplot(x='Lsr', y='Pwr', hue='Mouse', order=['PRE', 'LSR'], data=bdf, color='black', markers='', ci=None, ax=axs[i])
    ...
    # print stats
    pwaves.pairT_from_df(df, condition_col='Lsr', condition1=1, condition2=0, test_cols=band_labels, c1_label='during laser', c2_label='pre-laser', test_col_labels=clabs)

    


def laser_brainstate(ppath, recordings, pre, post, pplot=True, fig_file='', start_time=0, end_time=-1, flatten_tnrem=False, ma_state=3,
                     ma_thr=0, offset=0, sf=0, cond=0, edge=0, single_mode=False, backup='', csv_file='', ci='sem'):
    """
    ...
    for rec in recordings:
        ...
        M = load_stateidx(ppath, rec)[0]
        M = AS.adjust_brainstate(M, dt, ma_thr, ma_state, flatten_tnrem)
        ...
        (idxs, idxe) = laser_start_end(load_laser(ppath, rec), offset=offset)
        idf = re.split('_', rec)[0]
    ...
    nmice = imouse
    if pplot:
        state_label = {0:'REM', 1:'Wake', 2:'NREM', 3:'tNREM', 4:'fail_tNREM'}
        ...
        if not single_mode:
            ...
            colors = ['cyan', 'purple', 'gray', 'darkblue', 'red']
            #for state in [2,1,0]:
            for state in [4,3,2,1,0]:
                if ci == 95:
                    BS2 = BS[:,:,state].reshape(-1,order='F')  # get flattened timecourse mx of the form [mouse1time0, m2t0, m3t0, m1t1, m2t1, m3t1, m1t2 ...])
                    t2 = np.repeat(t, BS.shape[0])             # get repeated time vector of the form    [time0,         t0,   t0,   t1,   t1,   t1,   t2 ...]
                    sns.lineplot(x=t2, y=BS2, color=colors[state], ci=95, err_kws={'alpha':0.4, 'zorder':3}, linewidth=3, ax=ax)
                else:
        ...
        my_map = cmap.from_list('ha', [[0,1,1],[0.5,0,1], [0.6, 0.6, 0.6], [0.1,0.1,0.5], [1,0,0]], 5)
        x = list(range(Trials.shape[0]))
        #plt.pcolormesh(t,np.array(x), np.flipud(Trials), cmap=my_map, vmin=1, vmax=3)
        #plt.pcolormesh(t,np.array(x), np.flipud(Trials), cmap=my_map, vmin=1, vmax=5, shading='auto')
    ...
    S = ['REM', 'Wake', 'NREM', 'tNREM', 'fail_tNREM']
    ...
    lsr_char = pd.Series(['LSR']*nmice + ['PRE']*nmice + ['POST']*nmice, dtype='category')
    ...
    for i, state in enumerate(S):
        state_perc = np.concatenate((BS[:,ilsr,i].mean(axis=1), BS[:,ibase,i].mean(axis=1), BS[:,iafter,i].mean(axis=1)))*100  # state % values
        state_label = [state]*len(state_perc)
        # each state has a column with % time values (for paired t-tests)
        df[state]  = state_perc  
        # the 'perc' column holds all % time values, and the 'state' column holds the state label
        df2 = df2.append(pd.DataFrame({'Mouse' : mice, 'State' : state_label, 'Perc' : state_perc, 'Lsr' : lsr_char}))

    # create bar plot with % time spent in each state during pre-laser vs. laser interval
    plot_bars=True
    if ci == 'sem': ci = 68
    if plot_bars:
        plt.figure()
        ...
        for i in range(len(S)-1):
            ...
            sns.barplot(x='Lsr', y='Perc', order=['PRE', 'LSR'], data=sdf, ci=ci, palette={'PRE':'gray', 'LSR':'blue'}, ax=axs[i])
            #sns.boxplot(x='Lsr', y='Perc', order=['PRE', 'LSR'], data=sdf, fliersize=0, palette={'PRE':'gray', 'LSR':'blue'}, ax=axs[i])
            sns.pointplot(x='Lsr', y='Perc', hue='Mouse', order=['PRE', 'LSR'], data=sdf, color='black', markers='', ci=None, ax=axs[i])
            axs[i].get_legend().remove()
    ...
    pwaves.pairT_from_df(df, condition_col='Lsr', condition1=1, condition2=0, test_cols=S, c1_label='during-laser', c2_label='pre-laser', test_col_labels=clabs)
...
def laser_brainstate_bootstrap(ppath, recordings, pre, post, edge=0, sf=0, nboots=1000, alpha=0.05, backup='',
                               start_time=0, ma_thr=20, ma_state=3, flatten_tnrem=False, bootstrap_mode=0, fig_file=''):
    """
    ...
    for rec in recordings:
        ...
        M = load_stateidx(ppath, rec)[0]
        M = AS.adjust_brainstate(M, dt=dt, ma_thr=ma_thr, ma_state=ma_state, flatten_tnrem=flatten_tnrem)
    ...
    for mouse in BrainstateDict:
        Trials[mouse] = np.zeros((BrainstateDict[mouse].shape[0], len(t), 5))
    ...
    if bootstrap_mode == 1:
        ...
        mx_iter = np.zeros((ntrials, len(t), 5))
        for b in range(nboots):
    ...
    Prob_mean = np.zeros((nmice, len(t), 5))
    for s in [1,2,3,4,5]:
        i = 0
        for mouse in mice:
    ...
    colors = ['cyan', 'purple', 'gray', 'darkblue', 'red']
    br_states = {1:'REM', 2:'Wake', 3:'NREM', 4:'tNREM', 5:'failed tNREM'}
    ...
    labels = {1:'REM', 2:'Wake', 3:'NREM', 4:'tNREM', 5:'failed tNREM'}
    for s in [1,2,3,4,5]:
...
#                                start_time=0, ma_thr=20, bootstrap_mode=0, fig_file='', offset=0):
#     """
#     Align brain state with laser stimulation and calculate two-sided 1-$alpha confidence intervals using
#     bootstrapping.
#     :param ppath: base folder
...
#                                start_time=0, ma_thr=20, fig_file='', offset=0):
#     """
#     Align brain state with laser stimulation and calculate two-sided 1-$alpha confidence intervals using
#     bootstrapping.
...
def sleep_example(ppath, name, tlegend, tstart, tend, fmax=30, fig_file='', vm=[], ma_thr=10,
                  fontsize=12, cb_ticks=[], emg_ticks=[], r_mu = [10, 100], fw_color=True, pemg_ampl=False, 
                  raw_ex = [], eegemg_legend=[], eegemg_max=[], offset=0):
    """
    plot sleep example
    :param ppath: base folder
    :param name: recording name
    ...
    if plaser:
        laser = load_laser(ppath, name)
        idxs, idxe = laser_start_end(laser, SR=sr, offset=offset)
        idxs = [int(i / nbin) for i in idxs]
        idxe = [int(i / nbin) for i in idxe]
...
def sleep_timecourse(ppath, trace_file, tbin, n, tstart=0, tend=-1, pplot=True, stats='perc', csv_file=''):
    ...
    if pplot:
        ...
        #plt.bar(t, TimeMxCtr[1].mean(axis=1), yerr = TimeMxCtr[1].std(axis=1),  color='gray', fmt = 'o', linestyle='-', linewidth=2, elinewidth=2)
...
def sleep_spectrum_pwaves(ppath, recordings, win_inc=0.5, win_exc=1, istate=1, 
                          pnorm=False, nsr_seg=2, perc_overlap=0.95, recalc_highres=False, 
                          fmax=20, tstart=0, tend=-1, ma_thr=20, ma_state=3, 
                          flatten_is=4, noise_state=0, ci='sd', p_iso=0, pcluster=0,
                          clus_event='waves', exclude_noise=False, pplot=True):
    """
    Calculate power spectral density (PSD) for brain state with and without P-waves
    @Params
    ppath - base folder
    recordings - list of recordings
    win_inc - time window (s) surrounding each P-wave to include as "state with P-waves"
    win_exc - time window (s) surrounding each P-wave to exclude from "state w/o P-waves"
    istate - brain state to analyze
    pnorm - if True, normalize each frequency by its mean across the recording
    ...
    fmax - maximum frequency in analyzed PSDs
    ...
    noise_state - brain state to assign manually annotated bins of EEG/LFP noise
                  (if 0, do not analyze)
    ...
    exclude_noise - if False, ignore manually annotated EEG/LFP noise indices
                    if True, exclude time bins containing EEG/LFP noise from analysis
    pplot - if True, plot PSDs
    @Returns
    df - dataframe containing spectral power for time bins with and without P-waves
    """
    # collect mean spectrogram during P-waves (1) and no P-waves (0)
    ...
    for irec,rec in enumerate(recordings):
        print(f'Getting data for {rec} ({irec+1}/{len(recordings)})')
        idf = re.split('_', rec)[0]
        # create key for each mouse in data dictionaries
        if not idf in sp_data[0]:
            sp_data[0][idf] = []
            count_data[0][idf] = 0
            if cluster_mode == 0:
                sp_data[1][idf] = []
        ...
        if exclude_noise:
            ...
            LFP, p_idx, lfp_noise_idx, _, eeg_noise_idx = pwaves.load_pwaves(ppath, rec, 
                                                                 return_lfp_noise=True, 
                                                                 return_eeg_noise=True)
            noise_idx = np.unique(np.concatenate((lfp_noise_idx, eeg_noise_idx)))
        else:
            LFP, p_idx = pwaves.load_pwaves(ppath, rec)[0:2]
        # isolate single or clustered P-waves
        if p_iso and pcluster:
            # compare single and cluster P-waves
            iso_idx = pwaves.get_p_iso(p_idx, sr, win=p_iso)
            clus_idx = pwaves.get_pclusters(p_idx, sr, win=pcluster, return_event=clus_event)
        elif p_iso:
        ...
        seqs_dn = [range(int(round(seq[0]*cf)), int(round(seq[-1]*cf+cf))) for seq in seqs]
        sidx = np.concatenate(seqs_dn)
        
        # get indices of P-wave in $istate in SP time resolution
        p_idx_dn = np.round((p_idx + spi_adj[p_idx])/spnbin).astype('int')
        state_p_idx_dn = np.intersect1d(p_idx_dn, sidx)
        if cluster_mode == 1:
            iso_dn = np.round((iso_idx + spi_adj[iso_idx])/spnbin).astype('int')
            state_iso_dn = np.intersect1d(iso_dn, sidx)
            clus_dn = np.round((clus_idx + spi_adj[clus_idx])/spnbin).astype('int')
            state_clus_dn = np.intersect1d(clus_dn, sidx)
        # if P-wave "window" encompasses multiple SP bins, include all bins in "P-wave" condition
        if win_inc/spdt > 1:
            iwin_inc = int(round(win_inc/spdt))
            p_seqs = [range(pdn - iwin_inc, pdn + iwin_inc + 1) for pdn in state_p_idx_dn]
            state_p_idx_dn = np.concatenate(p_seqs)
            if cluster_mode == 1:
                iso_seqs = [range(idn - iwin_inc, idn + iwin_inc + 1) for idn in state_iso_dn]
                state_iso_dn = np.concatenate(iso_seqs)
                clus_seqs = [range(cdn - iwin_inc, cdn + iwin_inc + 1) for cdn in state_clus_dn]
                state_clus_dn = np.concatenate(clus_seqs)
                
            # check if exclusion window for "no P-wave" condition exceeds the "P-wave" window
            if (win_exc - win_inc) > spdt:
                iwin_exc = int(round(win_exc/spdt))
                i_exc = np.concatenate([range(pdn - iwin_exc, pdn + iwin_exc + 1) for pdn in state_p_idx_dn])
            else:
                i_exc = np.array(state_p_idx_dn)
        else:
            i_exc = np.array(state_p_idx_dn)

        # "no P-wave" bins = all state indices without P-waves and outside exclusion window
        state_nop_idx_dn = np.setdiff1d(sidx, i_exc)
        # get indices of noise in SP time
        if exclude_noise:
            noise_idx_dn = np.round((noise_idx + spi_adj[noise_idx])/spnbin).astype('int')
            #state_noise_idx_dn = np.intersect1d(noise_idx_dn, sidx)
        else:
            noise_idx_dn = np.array(()).astype('int')
        
        # create vector with len(t) bins
        ptrain_dn = np.zeros(len(t))
        
        # mark "P-waves"
        if cluster_mode == 0:
            if max(state_p_idx_dn) >= len(t):
                state_p_idx_dn = state_p_idx_dn[np.where(state_p_idx_dn < len(t))[0]]
            ptrain_dn[state_p_idx_dn] = 1    # 1 = P-wave(s)
        elif cluster_mode == 1:
            if max(state_iso_dn) >= len(t):
                state_iso_dn = state_iso_dn[np.where(state_iso_dn < len(t))[0]]
            if max(state_clus_dn) >= len(t):
                state_clus_dn = state_clus_dn[np.where(state_clus_dn < len(t))[0]]
            ptrain_dn[state_iso_dn] = 3    # 3 = single P-wave(s)
            ptrain_dn[state_clus_dn] = 4   # 4 = cluster P-wave(s)
        # mark "no P-waves"
        if max(state_nop_idx_dn) >= len(t):
            state_nop_idx_dn = state_nop_idx_dn[np.where(state_nop_idx_dn < len(t))[0]]
        ptrain_dn[state_nop_idx_dn] = 2  # 2 = no P-waves (outside exclusion window)
        # mark "noise"
        if noise_idx_dn.size > 0 and max(noise_idx_dn) >= len(t):
            noise_idx_dn = noise_idx_dn[np.where(noise_idx_dn < len(t))[0]]
        ptrain_dn[noise_idx_dn] = -1     # -1 = LFP and/or EEG noise
        
        # normalize SP, collect frequencies up to $fmax
        if pnorm:
            sp_mean = np.nanmean(SP, axis=1)
        ...
        # collect spectrum for SP bins with/without P-waves
        nop_sp = SP[:, np.where(ptrain_dn==2)[0]].sum(axis=1)
        sp_data[0][idf].append(nop_sp)
        count_data[0][idf] += len(np.where(ptrain_dn==2)[0])
        if cluster_mode == 0:
            p_sp = SP[:, np.where(ptrain_dn==1)[0]].sum(axis=1)
            sp_data[1][idf].append(p_sp)
            count_data[1][idf] += len(np.where(ptrain_dn==1)[0])
        elif cluster_mode == 1:
            iso_sp = SP[:, np.where(ptrain_dn==3)[0]].sum(axis=1)
            sp_data[3][idf].append(iso_sp)
            count_data[3][idf] += len(np.where(ptrain_dn==3)[0])
            clus_sp = SP[:, np.where(ptrain_dn==4)[0]].sum(axis=1)
            sp_data[4][idf].append(clus_sp)
            count_data[4][idf] += len(np.where(ptrain_dn==4)[0])

    mice = list(sp_data[0].keys())
    
    #sp_mx = {0:[], 1:[]}
    if cluster_mode == 0:
        conds, cond_labels = [[1,0], ['yes','no']]
        pal = {'yes':'blue', 'no':'gray'}
    elif cluster_mode == 1:
        conds, cond_labels = [[3,4,0], ['single','cluster','no']]
        pal = {'single':'dodgerblue', 'cluster':'darkblue', 'no':'gray'}
    sp_mx = {cond:[] for cond in conds}
        
    df = pd.DataFrame(columns=['Idf', 'Freq', 'Pow', 'P-wave'])
    #for plabel, pcond in zip(['yes','no'], [1,0]):
    for plabel, pcond in zip(cond_labels, conds):
        # assemble matrix of mice x freqs for P-wave/no P-wave bins
        mx  = np.zeros((len(mice), len(freq)))
        ddf = pd.DataFrame(columns=['Idf', 'Freq', 'Pow', 'P-wave'])
        for mrow, mname in enumerate(mice):
            # calculate avg power of each frequency for each mouse in condition $pcond
            d = np.array(sp_data[pcond][mname]).sum(axis=0) / count_data[pcond][mname]
            mx[mrow,:] = d
            ddf = pd.concat([ddf, pd.DataFrame({'Idf':mname, 
                                                'Freq':freq, 
                                                'Pow':d, 
                                                'P-wave':plabel})], 
    ...
    if pplot:
        ...
        # elif type(ci) == int:
        #     ci = ('ci',ci)
        #sns.lineplot(data=df, x='Freq', y='Pow', hue='P-wave', ci='sd', err_style='bars', palette=pal)
        ci = 68 if ci=='sem' else ci
        sns.lineplot(data=df, x='Freq', y='Pow', hue='P-wave', ci=ci, palette=pal)
            ...
            #sns.lineplot(data=df, x='Freq', y='Pow', hue='P-wave', ci=68, palette=pal)
        #else:
            #sns.lineplot(data=df, x='Freq', y='Pow', hue='P-wave', ci=ci, palette=pal)
        sns.despine()
        # set axis params
...
def sleep_spectrum(ppath, recordings, istate=1, pmode=1, fres=1/3, ma_thr=20.0, f_max=30, pplot=True, sig_type='EEG', mu=[10, 100],
                   ...
                   offset=0):
    """
    calculate spectrum for brain state i state for the given recordings.
    ...
    sig_type -    string, if 'EMG' calculate EMG amplitude (from the EMG spectrum). E.g.,
                  sleep_spectrum(ppath, E, istate=2, f_max=30, sig_type='EMG')
...
def sleep_spectrum_simple(ppath, recordings, istate=1, tstart=0, tend=-1, fmax=-1, ma_thr=20, ma_state=3, flatten_is=4, noise_state=0,
    ...
    for rec in recordings:
        print('Getting power spectrum for ' + rec + ' ...')
        ...
        if pmode == 2:
            LFP, p_idx = pwaves.load_pwaves(ppath, rec)[0:2]
            tmp = load_stateidx(ppath, rec)[0]
            pcount = pwaves.downsample_pwaves(LFP, p_idx, sr=sr, nbin=nbin, rec_bins=len(tmp))[2]
            pcount = pcount[istart:iend]
            
            idx_pwave = np.intersect1d(idx, np.where(pcount > 0)[0])
            idx_nopwave = np.intersect1d(idx, np.where(pcount == 0)[0])
        ...
        if pmode == 1:
            ...
            count_mice[0][idf] += len(idx_nopwave)
            count_mice[1][idf] += len(idx_pwave)
            ps_pwave   = SP[:,idx_pwave].sum(axis=1)
            ps_nopwave = SP[:,idx_nopwave].sum(axis=1)
        
            ps_mice[1][idf].append(ps_pwave)
            ps_mice[0][idf].append(ps_nopwave)
            amp_mice[1][idf] += emg_ampl[idx_pwave].sum()
            amp_mice[0][idf] += emg_ampl[idx_nopwave].sum()
    ...
    COND = 'P-wave' if pmode==2 else 'Lsr' 
    df = pd.DataFrame(columns=['Idf', 'Freq', 'Pow', COND], data=data)
    df_amp = pd.DataFrame(columns=['Idf', 'Amp', COND])
    if pmode > 0:
        df_amp['Idf'] = mice*2
        df_amp['Amp'] = list(amp_mx[0]) + list(amp_mx[1])
        df_amp[COND] = ['no'] * len(mice) + ['yes'] * len(mice)
    else:
        df_amp['Idf'] = mice
        df_amp['Amp'] = list(amp_mx[0]) 
        df_amp[COND] = ['no'] * len(mice) 
    ...
    if pplot:
        ...
        if ci == 'sem':
            ...
            # plt.plot(freq, m, color='blue')
            # plt.fill_between(freq, m-c, m+c, color='blue', alpha=0.2)
            sns.lineplot(data=df, x='Freq', y='Pow', hue=COND, ci=68, palette={'yes': 'blue', 'no': 'gray'})
        else:
            sns.lineplot(data=df, x='Freq', y='Pow', hue=COND, ci=ci, palette={'yes': 'blue', 'no': 'gray'})
        ...
        if ci == 'sem':
            ci = 68
        sns.barplot(data=df_amp, x=COND, y='Amp', palette={'yes':'blue', 'no':'gray'}, ci=ci)
        #sns.swarmplot(data=df_amp, x='Lsr', y='Amp', hue='Idf', palette='husl') 
        sns.lineplot(data=df_amp, x=COND, y='Amp', hue='Idf', palette='husl')
...
# def sleep_spectrum_simple(ppath, recordings, istate=1, tstart=0, tend=-1, fmax=-1, mu=[10,100], ci='sd', pmode=1, pnorm = False, 
#                           ma_thr=20, ma_state=3, noise_state=0, flatten_tnrem=False, pplot=True, harmcs=0, pemg2=False, exclusive_mode=False, 
#                           csv_files=[], offset=0, ylims=[]):
#     """
#     caluclate EEG power spectrum using pre-calculate spectogram save in ppath/sp_"name".mat
...
#     :param pemg2: if True, use EMG2 for EMG amplitude calcuation
#     :param exclusive_mode: if True, dissect out sections of brainstate where laser is on vs off
#                            if False, count brainstate sequences that have any overlap with laser as "laser-triggered"
#     :param csv_files: if two file names are provided, the results for EEG power spectrum
#     and EMG amplitude are saved to the csv files. The EEG powerspectrum is
#     saved to the first file.
...
#         # load brainstate
#         M = load_stateidx(ppath, rec)[0]
#         M = AS.adjust_brainstate(M, dt, ma_thr=ma_thr, ma_state=ma_state, noise_state=noise_state, flatten_tnrem=flatten_tnrem)
...
#         if pmode == 1:
#             lsr = load_laser(ppath, rec)
#             idxs, idxe = laser_start_end(lsr[istart_eeg:iend_eeg], offset=offset)
...
#             plt.figure()
#             sns.set_style('ticks')
#             sns.lineplot(data=sdf, x='Freq', y='Pow', hue='Lsr', ci=ci, palette={'yes':'blue', 'no':'gray'})
...
#             # plt.figure()
#             # plt.axes([0.1, 0.1, 0.4, 0.8])
#             # sns.barplot(data=sdf_amp, x='Lsr', y='Amp', palette={'yes':'blue', 'no':'gray'}, ci=ci)
#             # #sns.swarmplot(data=df_amp, x='Lsr', y='Amp', hue='Idf', palette='husl') 
#             # sns.lineplot(data=sdf_amp, x='Lsr', y='Amp', hue='Idf', palette='husl')
#             # plt.ylabel('Amp ($\mathrm{\mu V}$)')
#             # sns.despine()
#             # plt.show()
...
def _whole_mx(ppath, name, pre, post, tdown, ptie_break=1, tstart=0, tend=-1, offset=0):
    """
    @Return:
    ...
    (idxs, idxe) = laser_start_end(load_laser(ppath, name), offset=offset)
...
###   MANDY ADDITIONS   ###
def sleep_state_highres(ppath, name, th_delta_std=1, mu_std=0, sf=1, sf_delta=3, pwrite=0, pplot=False, pemg=False, vmax=2.5, pspec_norm=False, use_idx=[], psave=True):
    """
    automatic sleep state detection based on
    delta, theta, sigma, gamma and EMG power.
    ...
    #load EEG and EMG spectrum, calculated by calculate_spectrum
    P = so.loadmat(os.path.join(ppath, name,  'sp_highres_' + name + '.mat'))
    if pemg:
        Q = so.loadmat(os.path.join(ppath, name, 'msp_' + name + '.mat'))
    else:
        #Q = so.loadmat(os.path.join(ppath, name, 'sp_' + name + '.mat'))
    ...
    if psave:
        so.savemat(os.path.join(ppath, name, 'remidx_highres_%s.mat' % name), {'M' : M, 'S' : S})
    # write sleep annotation to file
    if pwrite:
        outfile = os.path.join(ppath, name, 'remidx_' + name + '.txt')
        print("writing annotation to %s" % outfile)
...
def read_sev(ppath, audio_file=[]):
    import struct
    if len(audio_file)==0:
        sev_file = ppath
    else:
        sev_file = os.path.join(ppath, audio_file)
    
    #if this path leads to an actual file, print "importing data from single SEV file"
    #if os.path.isfile(os.path.join(ppath, audio_file)):
    if os.path.isfile(sev_file):
        fileName, fileExtension = os.path.splitext(sev_file)
        #if this file is not actually a SEV file, print "not a valid SEV file"
    ...
    #open the sev file
    f = open(sev_file, 'rb')
    #read the first 40 characters of the sev file
    head = f.read(40)
    
    #unpack all the fun information stored in the sev file
    ...
    #yep, should still be a sev file
    if fileType.lower() != 'sev':
        print('bad file type', fileType)
        #return None 
        
    #fileVersion should be 2
    if fileVersion > 2:
    ...
    if fileVersion > 0:
        # determine data sampling rate - should be 1525.88, to match EEG/LFP sampling rate
    ...
    #make a dictionary and fill it with keys containing all the information we unpacked from the SEV file
    d = dict()
    ...
    d['fs']               = fs
    d['fmt']              = fmt
    #THIS IS THE AUDIO STIMULATION INFORMATION
...
# rem_online_analysis: added 'sem' parameter to plot error bars instead of individual trial, ylim parameter to standardize y axes, and perform independent t-test
# laser_triggered_eeg_avg: added 'plot_EEGonly' parameter to limit plotting to EEG spectrogram only.  It's pretty useless
...
def phasic_emg(ppath, name, min_dur=2.5, ndown=50, pplot=False, min_phasic=0.1, 
    ...
    ppath - base folder
    name - recording name
    min_dur - minimum REM duration
    ndown - downsampling factor for raw EMG
    min_phasic - minimum time (s) of EMG activation that constitutes a phasic event
    break_dur - minimum time (s) separating consecutive, distinct EMG twitches
    thr_perc - percentile of EMG activation used to detect phasic REM
    """
    mu = [50, 200]
    M = load_stateidx(ppath, name)[0]
    ...
    # find REM sleep
    neeg = EMG.shape[0]  # no. samples in raw EMG
    seq = get_sequences(np.where(M == 1)[0])
    rem_idx = []
    for s in seq:
        rem_idx += list(s)
    ...
    nbin = int(np.round(2.5*sr))
    sdt = nbin*(1/sr)
    ibreak = int(break_dur / (ndown*(1/sr))) + 1  # min bins separating distinct twitch events
    dt_hp = (1/sr)*ndown  # s per bin in downsampled EMG

    seq = [s for s in seq if len(s)*sdt>=min_dur]  # qualifying REM sequences longer than $min_dur
    hp_emg_list = []
    hp_seq = {}
    for s in seq:
        ta = s[0]*nbin  # starting REM idx (Intan EMG resolution)
        
        #[0      1         2         3]
        #[0-2500 2500-5000 5000-7500 7500 9999]
        tb = (s[-1]+1)*nbin  # ending REM idx (Intan EMG resolution)
        tb = np.min((tb, neeg))

        emg_idx = np.arange(ta, tb)        
        emg_cut = EMG[emg_idx]  # raw EMG during REM sequence
        if len(emg_cut)*(1/sr) <= min_dur:
            continue

        emg_cut = np.abs(my_hpfilter(emg_cut, 0.5))  # highpass filter (cutoff freq = w0 * (sr/2) = 250 Hz)
        emg_cut = downsample_vec(emg_cut, ndown)     # downsample EMG (new sampling rate = sr/ndown; e.g. if ndown=50, new sr=20, new Nyquist freq=10 Hz)
        emg_cut = np.abs(my_hpfilter(emg_cut, 0.1))  # highpass filter again (cutoff freq = w0 * (new sr/2) = 1 Hz)

        hp_emg_list += list(emg_cut)  # collect highpassed EMG
        hp_seq[s[0]] = emg_cut  # save highpassed EMG to starting REM idx in annotation

    hp_emg = np.array(hp_emg_list)
    # threshold for phasic REM detection:
    thr = np.percentile(hp_emg, thr_perc)  # get 99th percentile of EMG activation for ALL REM periods
    
    print('Threshold for recording %s: %f' % (name, thr))

    phemg = {}
    ph_idx_all = []
    for si in hp_seq:  # iterate over all REM starting indices
        offset = nbin * si  # starting idx in raw EMG
        emg_cut = hp_seq[si]
        idx = np.where(emg_cut > thr)[0]  # indices where downsampled EMG exceeds 99th percentile
        ...
        for q in cand:  # iterate over all twitch events in REM sequence
            dur = len(q) * dt_hp  # duration of EMG activation (s)

            if dur < min_phasic:  # qualifying EMG twitches longer than $min_phasic
                continue
            
            a = q[0]*ndown + offset  # starting idx of twitch in original raw EMG
            b = (q[-1]+1)*ndown + offset  # ending idx in original raw EMG

            ph_idx = range(a, b)
            
            # save twitch indices in original (unfiltered, not downsampled) EMG
            if si in phemg:
            ...
            # save corresponding indices in downsampled EMG
    ...
    if pplot:
        ...
        # load or calculate SP and mSP with given FFT parameters
        file_sp  = os.path.join(ppath, name,  'sp_fine_%s.mat' % name)
        ...
        SP   = tmp['SP']
        freq = tmp['freq']
        dfreq = freq[1] - freq[0]  # 0.5 (frequency resolution)
        tbs    = tmp['t']
        dt   = tmp['dt']
    
        tmp  = so.loadmat(file_msp, squeeze_me=True)
        mSP = tmp['mSP']
        # eliminate frequencies outside $mu freq band
        imu = np.where((freq >= mu[0]) & (freq <= mu[1]))[0]
        # sum desired frequencies, multiply by freq resolution to get EMG amp
        ampl = mSP[imu,:].sum(axis=0) * dfreq
        ampl = my_hpfilter(ampl, 0.2)  # highpass filter EMG amplitude
        ampl = np.abs(ampl)        
        ...
        for si in hp_seq:
            ...
            plt.plot(teeg_hp[idx_hp], emg_hp)  # plot highpassed, downsampled EMG used for detection

            plt.plot((teeg_hp[idx_hp[0]], teeg_hp[idx_hp[-1]]), np.ones(2,)*thr, 'r')  # plot detection threshold
        ...
        for s in seq:
            ...
            plt.plot(teeg_dn[idx_dn], EMGdn[idx_dn], 'k', lw=0.5)  # plot original, raw EMG for REM sequence
        ...
        for si in phemg:
            ...
            for idx in idx_list:
                ...
                plt.plot(teeg_dn[range(a,b+1)], EMGdn[a:b+1], 'r', lw=0.5)  # plot phasic EMG periods in original EMG
